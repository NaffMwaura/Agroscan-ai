...
import { useLocation, useNavigate, Link } from 'react-router-dom'; // ADDED Link component
...
// --- Helper Function: Format API Errors ---
...
const AuthPage: React.FC<AuthPageProps> = ({ onLoginSuccess }) => {
    ...
    // Initialize view state based on URL, or default to 'login'
    ...
    useEffect(() => {
        ...
        setMessage(null); // Clear messages on route change
    }, [location.pathname]); // Dependency on the path
    ...
    const handleRegister = useCallback(async () => {
        ...
        try {
            ...
            if (response.ok && data) {
                ...
                // Redirect on success is handled programmatically here:
                navigate('/login'); 
            } else {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center font-inter">
            ...
            {/* Fallback for small screens (Mobile/Tablet) */}
            <div className="lg:hidden bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border-t-4 border-green-600 mx-auto mt-24 mb-8">
                <div className="text-center">Mobile/Tablet Form Here</div>
            </div>
        </div>
    );
};
